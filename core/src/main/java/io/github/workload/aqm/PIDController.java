...
class PIDController {
     ...
     * </p>
     *
     * @param sample 当前系统的实际测量值
     * @return 调整系数，[0, 1]。0表示不需要调整：期望值与实际值完全相同
     */
    public double compute(double sample) {
        final double error = setpoint - sample; // 计算偏差
        integral += error; // 更新积分项
        final double derivative = error - lastError; // 计算微分项
        lastError = error;
