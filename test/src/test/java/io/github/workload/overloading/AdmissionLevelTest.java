...
class AdmissionLevelTest {
    ...
    void immutable() {
        ...
        assertNotEquals(P, level.changeBar(WorkloadPriority.of(1, 2).P()).P());
        assertEquals(WorkloadPriority.ofLowestPriority().P(), level.P());
        assertEquals(WorkloadPriority.of(5, 6).P(), level.changeBar(WorkloadPriority.of(5, 6).P()).P());
    }
    ...
    void switchTo() {
        AdmissionLevel level = AdmissionLevel.ofAdmitAll();
        // P 没变，则返回当前实例
        assertSame(level, level.changeBar(WorkloadPriority.ofLowestPriority().P()));
        WorkloadPriority priority = WorkloadPriority.ofPeriodicRandomFromUID(1, 1, 120);
        assertTrue(level.admit(priority));
        level.changeBar(WorkloadPriority.of(0, 0).P());
        // changeTo will not change level, but return a new level
        assertEquals(WorkloadPriority.ofLowestPriority().P(), level.P());
        AdmissionLevel level1 = level.changeBar(WorkloadPriority.of(0, 0).P());
        assertEquals(0, level1.P());

        level = level.changeBar(WorkloadPriority.of(3, 20).P());
    ...
    void testToString() {
        ...
        level = level.changeBar(WorkloadPriority.of(5, 9).P());
