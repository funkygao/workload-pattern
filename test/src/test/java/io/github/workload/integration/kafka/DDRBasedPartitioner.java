...
@WIP
public class DDRBasedPartitioner implements Partitioner {
    private Map<Integer /* partition */, AtomicInteger> partitionDeficits = new ConcurrentHashMap<>();
    ...
    public int partition(String topic, Object key, byte[] keyBytes, Object value, byte[] valueBytes, Cluster cluster) {
        ...
        for (int partition = 0; partition < partitions; partition++) {
            // 初始化该分区的信用额度
            partitionDeficits.putIfAbsent(partition, new AtomicInteger(0));

            final int deficitWeight = getPriorityWeight(priority);
            final AtomicInteger partitionDeficit = partitionDeficits.get(partition);
        ...
        AtomicInteger selectedPartitionDeficit = partitionDeficits.get(selectedPartition);
        final int deficitWeight = getPriorityWeight(priority);
    ...
    void demo() {
        ...
        DDRBasedPartitioner partitioner = new DDRBasedPartitioner();
